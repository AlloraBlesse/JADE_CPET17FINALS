...
app.get('/', function (req, res) {
    ...
    var user_table = ("CREATE TABLE IF NOT EXISTS user_table (id INT AUTO_INCREMENT PRIMARY KEY, firstname VARCHAR(45), lastname VARCHAR(45), phone VARCHAR(45), username VARCHAR(45), email VARCHAR(45), password VARCHAR(100), confirmpassword VARCHAR(100))")
...
app.get("/displaycam", (req, res) => {
    connection.query(
        "SELECT * FROM `motion_table` ORDER BY id DESC LIMIT 4",
        function (err, results) {
            if (err) throw err;
            try {
                if (results.length > 0) {
